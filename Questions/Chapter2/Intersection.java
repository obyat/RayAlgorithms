...
public class Intersection{
    ...
    Note theat the intersection is defined based on reference (memory address) and not value. that is the kth node of the first linked list is the exact same
    node (by reference/memory address) as the jth node of the second linked list, then they are intersecting.

    Hints: 20 45 55 65 76 93 111 120 129
    */

    //Runtime: O(l1.size() + l2.size()) + O(min(l1.size(), l2.size())) runtime, + O(1) space + O(1) stack space
    public static DllNode intersection(DoublyLinkedList l1, DoublyLinkedList l2){
        ...
        if(l1LastNode.node != l2LastNode.node) return null;
    ...
    Ideas:
        1. HasSet, iterate through both linkedlists and when an address repeats, return the node O(L1 + L2)
        2. If DoublyLinkedLists, iterate through both linkedLists and check in reverse order O(L1 + L2)
        3. Traverse both linkedlist and compare the last node then use the difference to check both and return first same address of node
    ...
    Mistakes:
        1. writing middle portion of code first, then left, then right. Should start from left.
    */
